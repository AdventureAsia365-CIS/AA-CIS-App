...
class RawTourRepository:
    ...
    async def insert(self, data: dict) -> str:
        ...
        """,
            ...
            (lambda v: v if (v or "").strip().startswith("[")
             else json.dumps([v]) if v else "[]")(data.get("src_highlights")),
            ...
            data.get("exclusions"),
            (lambda v: v if (v or "").strip().startswith("[")
             else json.dumps([v]) if v else "[]")(data.get("links")),
            (lambda v: v if (v or "").strip().startswith("[")
             else json.dumps([v]) if v else "[]")(data.get("activities")),
            data.get("feature"),
