...
# AA-549 — `GET /quarter-plan/history` + `GET /quarter-plan/versions/{version_id}` (AA-469 Việc 2,
# tenant-facing quarter-plan version history/preview) REMOVED here. Confirmed orphaned (AA-547
# investigation, 06/09/2026): PlanningTab.tsx's own Preview/trip-table/finalize/history UI was
# fully superseded by the Slate (AA-511) and torn out at AA-519 (03/09/2026, Nghiệp's explicit
# call) — these 2 routes had 0 frontend caller left (grep-confirmed before deleting, see AA-549
# implementation notes). The underlying `services/acp_planning/quarter.py::
# fetch_quarter_plan_version()`/`fetch_quarter_plan_version_history()` are NOT deleted — out of
# this cleanup's scope (only these 2 routes were asked for); they are themselves now 0-caller
# too (their other historical caller, `admin_atoms.py`'s `GET /admin/atoms/preview-slotgrid`, was
# separately removed at AA-475) — flagged, not acted on, same conservative "evaluate before
# deleting an adjacent thing" stance this issue's own scope took for the `quarter_plan_version`
# DB table (still live, used by `save_quarter_plan_version()`/`approve_quarter_plan_version()`/
# `fetch_approved_quarter_plan()` — not orphaned, not touched).
