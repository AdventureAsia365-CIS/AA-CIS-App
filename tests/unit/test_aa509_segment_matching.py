...
(place/action columns added this same task).

AA-545 — `tenant_id` removed from `derive_segments()`/`_mint()` (platform-wide redesign);
`test_cross_tenant_same_content_does_not_collide_segment_id` below is renamed/flipped to
`test_same_content_across_tours_collides_on_purpose` — the collision it used to guard against is
now the intended behavior (see docs/implementation-notes/AA-545.md).

STEP0: docs/claude_audit/AA-509-step0-schema-matching-investigation.md.
Implementation notes: docs/implementation-notes/AA-509.md, docs/implementation-notes/AA-545.md.
"""
...
def test_same_place_different_action_not_merged():
    ...
    segments = derive_segments(atoms)
...
def test_same_content_across_tours_collides_on_purpose():
    """AA-545 — the exact opposite of AA-509's original `test_cross_tenant_same_content_does_
    not_collide_segment_id`: a real-world moment described identically by 2 different tours must
    derive the SAME segment_id (restoring the `recurrence` rank-sum axis, ADR 0014) — collision
    is the feature now, not a bug to avoid (see this module's own docstring)."""
    atoms_x = [_atom("a1", "tourX", 1, "Magome", "walk to Tsumago")]
    atoms_y = [_atom("a2", "tourY", 1, "Magome", "walk to Tsumago")]
    seg_x = derive_segments(atoms_x)[0]
    seg_y = derive_segments(atoms_y)[0]
    assert seg_x.id == seg_y.id
...
def test_reconcile_ids_stable_across_runs_with_no_new_atoms():
    ...
    first, _ = reconcile_ids(derive_segments(atoms), {})
    assigned = {atom_id: seg.id for seg in first for atom_id in seg.atom_ids}

    second, aliases = reconcile_ids(derive_segments(atoms), assigned)
...
def test_reconcile_ids_keeps_id_when_a_bridging_atom_merges_two_segments():
    ...
    once, on first sight); instead one of the two PRIOR ids is kept and the other comes back as
    an alias."""
    atoms_run1 = [
        _atom("a1", "tour1", 1, "the grand old temple", "visit"),
        _atom("a2", "tour2", 1, "the sacred temple ruins", "visit"),
    ]
    first, _ = reconcile_ids(derive_segments(atoms_run1), {})
    ...
    second, aliases = reconcile_ids(derive_segments(atoms_run2), assigned)
