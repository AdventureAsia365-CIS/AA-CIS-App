...
def _piece(body_tagged, piece_id="p1"):
    return Piece(piece_id=piece_id, body_tagged=body_tagged, status="in_progress")
...
async def test_piece_passing_all_gates_persists_passed_and_emits_metrics_but_not_usage_log():
    ...
    # gate_ledger persisted includes output_rules + F1 + F8 + F9, in that order
    gate_ledger = json.loads(params[7])
    assert [g["gate"] for g in gate_ledger] == [
        "output_rules", "F1_grounding", "F8_framework", "F9_brand_seo_audit",
    ]
