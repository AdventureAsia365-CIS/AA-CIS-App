...
const CONTENT_NAV = [
  { href: "/upload",          icon: <Upload size={15} />,       label: "Upload (S0)" },
  { href: "/pipeline/s1",     icon: <Wand2 size={15} />,        label: "S1 Rewrite" },
  { href: "/master-content",  icon: <Library size={15} />,      label: "Master Content" },
    ...
    <aside style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, paddingBottom: 18, borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{
          ...
          background: A.red, display: "grid", placeItems: "center",
          fontFamily: serif, fontWeight: 700, color: "#fff", fontSize: 14,
        }}>AA</div>
        <div>
          <div style={{ fontFamily: serif, fontSize: 14, fontWeight: 500, color: "#F4F1EC", letterSpacing: "-0.01em", lineHeight: 1.2 }}>
            CIS Admin
          </div>
          <div style={{ fontSize: 9.5, textTransform: "uppercase", letterSpacing: "0.18em", color: A.red, fontWeight: 600, marginTop: 1 }}>
            Internal
          </div>
        </div>
      </div>
        ...
        <NavGroup label="Admin">
          {ADMIN_NAV.map(n => (
            <NavItem key={n.href} active={pathname === n.href}
              icon={n.icon} label={n.label} accent={A.red}
      ...
      <div style={{ paddingTop: 14, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
        <div style={{
          ...
          <div style={{
            width: 30, height: 30, borderRadius: 6, background: A.red,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ color: "#8A929D", fontSize: 10.5 }}>Administrator</div>
          </div>
