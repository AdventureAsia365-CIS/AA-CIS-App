...
import { A, serif, sans, Card, SLabel, Btn, TopBar } from "../_components/internalUi";
...
export default function BrandPage() {
  ...
  async function save() {
    ...
    setSaving(false);
    if (r.ok) { setMsg("Saved ✓"); fetch("/api/tenant/pipeline/brand-identity").then(r=>r.json()).then(setData); }
    else setMsg("Save failed");
  }

  async function restore(v: NonNullable<BrandData["history"]>[0]) {
    setSp(v.system_prompt || ""); setSg(v.style_guide || "");
    setFw((v.forbidden_words || []).join(", "));
    setMsg(`Restored version ${v.version} — save to apply`);
  }

  if (!data) return <div style={{ padding: 40, color: A.muted }}>Loading…</div>;
